...
export async function parseSpreadsheet(file) {
  ...
  const sheets = workbook.SheetNames.map((sheetName) => {
    ...
    const rows = raw
      .slice(1)
      // Ignora linhas completamente vazias (linhas fantasmas do Excel)
      .filter((r) => r.some((cell) => cell !== null && cell !== undefined && String(cell).trim() !== ''))
      .map((r) =>
        headers.reduce((acc, h, i) => {
          acc[h] = r[i] !== undefined && r[i] !== null ? String(r[i]).trim() : '';
          return acc;
        }, {})
      )
      // Segunda passagem: remove linhas onde todos os campos mapeados são vazios
